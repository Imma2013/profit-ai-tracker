...
export default function Home() {
  ...
  const { loading: accessLoading, allowed } = usePaidAccess();
  ...
  const analyses = useQuery(
    api.analyses.getUserAnalyses,
    allowed && userId ? { userId } : "skip"
  );

  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !allowed || !auth.currentUser) return;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 px-5 py-8 pb-28 max-w-md mx-auto relative font-sans">
      ...
      <div className="fixed bottom-0 left-0 w-full pb-8 pt-3 px-8 bg-white/90 backdrop-blur-xl border-t border-slate-200 flex justify-between items-center z-50">
        ...
        <button onClick={() => router.push("/profile")} className="text-slate-400 flex flex-col items-center hover:text-slate-900 transition-colors px-4 font-bold">
          <span className="text-2xl mb-1">👤</span>
